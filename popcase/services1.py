...
def apply_naaccr_filters(qs, filters: dict):
    ...
    if geo_scope in ("neo15", "neo_15", "catchment15", "catchment_15"):
        ...
        qs = qs.filter(mid__in=Subquery(neo_pat_ids))
...
def _get_filtered_mid_subquery(filters: dict):
    return apply_naaccr_filters(NaaccrData.objects.all(), filters or {}).values("mid")


def _get_filtered_patient_ids(filters: dict):
    return list(
        apply_naaccr_filters(NaaccrData.objects.all(), filters or {})
        .values_list("mid", flat=True)
        .iterator(chunk_size=5000)
    )


def _stage_counts_by_geo(geographic_level: str, filters: dict):
    """
    Aggregates stage-based denominator / advanced / metastatic counts directly from the DB
    without materializing a full patient->stage mapping in Python.
    """
    filtered_mid_subquery = _get_filtered_mid_subquery(filters)

    if not NaaccrPatientCensusLinking.objects.filter(
        geographic_level=geographic_level,
        pat_id__in=Subquery(filtered_mid_subquery),
    ).exists():
        return {}, {}, {}

    # Keep patient filtering in the database, then do one narrow stage lookup in Python.
    # Avoid annotate(Subquery(...)) here: with unmanaged models and no FK relation, Django
    # can generate an exact-lookup subquery that expects a single row and raises
    # "The QuerySet value for an exact lookup must be limited to one result".
    stage_by_mid = dict(
        apply_naaccr_filters(NaaccrData.objects.all(), filters or {})
        .values_list("mid", "stg_grp")
        .iterator(chunk_size=5000)
    )
    ...
    for pat_id, geoid in linking_rows:
        ...
        geoid = _normalize_geoid_for_level_value(geoid, geographic_level)
        if not geoid or not _geoid_in_scope(geographic_level, geoid, filters):
            continue
    ...
    return dict(denom_by_geo), dict(adv_by_geo), dict(meta_by_geo)
...
def _get_incidence_by_geography_uncached(year, geographic_level, filters):
    ...
    case_lookup = {
        ...
        for r in case_counts.iterator(chunk_size=5000)
    }
    if not case_lookup:
        return []

    filtered_pat_ids = _get_filtered_patient_ids(filters)
...
def build_mvp_tract_dataset(
    ...
    denom_by_tract, adv_by_tract, meta_by_tract = _stage_counts_by_geo("tract", filters)
    ...
    for tract_geoid in all_tracts:
        ...
        if "case_count" in disease_measures:
            out["case_count"] = int(denom_by_tract.get(tract_geoid, incidence_lookup.get(tract_geoid, {}).get("case_count", 0)))
...
def _build_mvp_geo_dataset_uncached(
    ...
    denom_by_geo, adv_by_geo, meta_by_geo = _stage_counts_by_geo(geographic_level, filters)
